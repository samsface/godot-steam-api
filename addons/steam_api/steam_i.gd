...
class SteamUserStatsProxy_:
	...
	func indicate_achievement_progress(
		achievement_api_name: String, current_progress: int, max_progress: int
	) -> bool:
		return call_(
			"indicate_achievement_progress", [achievement_api_name, current_progress, max_progress]
		)
	...
	func find_or_create_leaderboard(leaderboard_name: String, sort_method: int, display_type: int):
		return callback_(
			"find_or_create_leaderboard", [leaderboard_name, sort_method, display_type]
		)
...
func set_leaderboard_score(
	leaderboard_name: String,
	score: int,
	method: int = LeaderboardUploadScoreMethod.KeepBest,
	details: PoolIntArray = PoolIntArray(),
	sort_method: int = -1,
	display_type: int = LeaderboardDisplayType.Numeric
):
	var callback := Callback.new()
	set_leaderboard_score_(
		leaderboard_name, score, method, details, sort_method, display_type, callback
	)
	return callback


func set_leaderboard_score_(
	leaderboard_name: String,
	score: int,
	method: int,
	...
	if sort_method > 0:
		find_leaderboard_result = yield(
			user_stats.find_or_create_leaderboard(leaderboard_name, sort_method, display_type),
			"done"
		)
	else:
...
func get_leaderboard_scores(
	...
	method: int = LeaderboardDataRequest.Global,
	max_details: int = 0
):
	var callback := Callback.new()
...
func get_leaderboard_scores_(
	leaderboard_name: String, begin: int, end: int, method: int, max_details, callback: Callback
) -> Callback:
	var res := []
	...
	var download = yield(
		user_stats.download_leaderboard_entries(leaderboard, method, begin, end), "done"
	)
	if not download:
		return callback.emit_signal("done", res)
	...
	for i in download.get_entry_count():
		...
		if entry.get_steam_id_user() and user.get_steam_id():
			...
			score["is_player"] = (
				user.get_steam_id().get_account_id()
				== entry.get_steam_id_user().get_account_id()
			)
		else:
			score["id"] = 0
