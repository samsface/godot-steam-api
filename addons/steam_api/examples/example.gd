...
func _ready():
	...
	yield(
		Steam.set_leaderboard_score(
			"High Score",
			1001,
			Steam.LeaderboardUploadScoreMethod.KeepBest,
			PoolIntArray([1, 2, 3]),
			Steam.LeaderboardSortMethod.Descending
		),
		"done"
	)

	var scores = yield(
		Steam.get_leaderboard_scores("High Score", 0, 10, Steam.LeaderboardDataRequest.Global, 3),
		"done"
	)
	print(scores)
