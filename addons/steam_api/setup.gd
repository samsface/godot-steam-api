...
func copy_file_from_steam_sdk_(base_path: String, file_name: String) -> void:
	...
	if not File.new().file_exists("res://addons/steam_api/%s" % file_name):
		var res := Directory.new().copy(
			settings.sdk_dir + base_path + file_name, "res://addons/steam_api/%s" % file_name
		)
		if res != OK:
			popup_error(
				(
					"Failed to copy %s. Steam integration will not work. Did you set the 'SDK Directory' correctly?"
					% file_name
				)
			)
...
func write_steam_app_id_file_() -> void:
	...
	if res == OK:
		...
		popup_error(
			"Failed to write steamapp_id.txt to \"{exe_path}\". Steam integration will not work. For debug builds, steam needs a text file with just your steam_app_id named 'steam_appid.txt' next to your exe in '{exe_path}'. Try write the file manually.".format(
				{"exe_path": exe_path_()}
			)
		)
...
func check_status():
	...
	var missing = []
	if not file.file_exists("res://addons/steam_api/libsteam_api.so"):
		missing.push_back("linux")
	if not file.file_exists("res://addons/steam_api/steam_api64.dll"):
		missing.push_back("windows")
	if not file.file_exists("res://addons/steam_api/libsteam_api.dylib"):
		missing.push_back("osx")
	if missing.size() > 0:
		if missing.size() == 3:
