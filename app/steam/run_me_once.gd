...
func copy_file_from_steam_sdk_(base_path:String, file_name:String) -> void:
	if not File.new().file_exists("res://steam/%s" % file_name):
		var res := Directory.new().copy(SteamOptions.sdk_dir + base_path + file_name, "res://steam/%s" % file_name)
		if res != OK:
			push_error("Failed to copy %s. Steam integration will not work. Did you set the sdk_dir correctly?" % file_name)
...
func write_steam_app_id_file_() -> void:
	...
	if res == OK:
		f.store_string(str(SteamOptions.app_id))
		f.close()
	else:
...
func steam_appid_file_has_correct_id_() -> bool:
	...
	if res == OK:
		return str(SteamOptions.app_id) != file.get_line()
	else:
		return false
